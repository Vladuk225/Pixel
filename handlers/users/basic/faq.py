...
async def faq_menu(call: CallbackQuery, _):
    ...
    await call.message.answer_photo(photo='https://imgur.com/a/lnBXf7S', caption=_("faq_menu_text"), reply_markup=faq_kb(call.from_user.id, _))
...
async def faq_menu(message: Message, _):
    await message.answer_photo(photo='https://imgur.com/a/lnBXf7S', caption=_("faq_menu_text"), reply_markup=faq_kb(message.from_user.id))
...
async def rules_menu(call: CallbackQuery, _):
    ...
    if not call.from_user.id == int(user_id_str):
        ...
        return await call.answer(msg, show_alert=True)
    await call.message.delete()
    await call.message.answer_photo(photo='https://imgur.com/a/UKUL7YD', caption=_("stats_info_text").format(await Stats.total_users(), await Stats.total_chats(), await Stats.banned_users(), await Stats.count_active_users_24h()), reply_markup=back_to_faq_menu_kb(call.from_user.id, _))
...
async def stats_info(call: CallbackQuery, _):
    ...
    await call.message.answer_photo(photo='https://imgur.com/a/9uU0pas', caption=f'''
{_("rules_menu")}
...
async def show_link(call: CallbackQuery, _):
    ...
    await call.message.answer_photo(
        photo='https://imgur.com/a/yc2z4IU',
        caption=f'''
{_("links_menu")}
...
async def info_menu(call: CallbackQuery, _):
    ...
    link = ru_doc_link if user_lang == 'ru' else en_doc_link
    await call.message.answer_photo(photo='https://imgur.com/a/9uU0pas', caption=_("doc_info").format(link), reply_markup=back_to_faq_menu_kb(call.from_user.id, _))