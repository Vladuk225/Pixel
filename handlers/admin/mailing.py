...
async def rass_step3_handler(message: Message, state: FSMContext):
    if message.text != '-':
        try:
            ...
            row_len = len(text[0].split(','))
            kb = InlineKeyboardMarkup(row_width=row_len)
            for row in text:
                for button in row.split(','):
                    name, url = button.strip().split('|')
                    kb.add(InlineKeyboardButton(text=name.strip(), url=url.strip()))
        except Exception:
            await message.answer("⚠️ Неверный формат клавиатуры. Пример:\nКупить|https://url\nПоддержка|https://url")
            return
    ...
    await message.answer(
        ...
        reply_markup=get_target_select_kb(set(), message.from_user.id)
    )
...
async def handle_target_selection(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "target_users":
        ...
        await call.message.edit_text("📅 Пришлите дату в формате (d.m.Y h:m) или '-' чтобы отправить сейчас.")
        return
...
async def rass_finish_handler(message: Message, state: FSMContext):
    ...
    if user_time == "-":
        ...
        if delay < 0:
            await message.answer("❗ Вы указали время в прошлом, отправка будет произведена сейчас.")
            delay = 0
    ...
    progress_msg = await message.answer("Отправлено: 0\nУспешно: 0\nНеуспешно: 0")
